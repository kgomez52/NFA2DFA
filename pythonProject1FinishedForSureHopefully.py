...
# -*- coding: utf-8 -*-
"""
@authors: iearnes, dbmart2, krgomez
"""

# Define the NFA example on the slide 9 of Finite Automata (part 3)
...
currentTest = NFA1
q0 = 'A'#make sure to manually change this to fit the current NFA based on its q0 lambda transitions
...
currVal =''
hardstop = 0 #temporary measure to save my poor pc
...
while not done and hardstop<100:#loop start, increase hardstop if necessary for a massive DFA
            ...
            if currVal[element] in key:
                for keySymbol, valueResult in value.items():
                    for val in valueResult:
                        if val not in temp3 and val is not NULL and val not in DFA[currVal][keySymbol]:#ignoring trap states for now
                            temp3+=val
